...
2. Ejecutar `migrations/202605300001_initial_orchid_pos_schema.sql` en el SQL Editor o con el CLI de Supabase solo si la base esta vacia.
...
- El backend actual usa Postgres con una capa de compatibilidad para placeholders `?`.
- Para bases existentes, usa `npm run db:migrate`. Ese comando marca la migracion inicial como aplicada cuando ya existe `productos` y aplica solo las migraciones nuevas.
